<template>
  <div class="agent-page">
    <div class="agent-header container">
      ...
      <h1 class="agent-title">客户流失预警</h1>
      <p class="agent-desc">识别沉睡客户，自动触发激活流程</p>
    </div>
...
const generate = async () => {
  ...
  result.value = null
  try {
    const data = await request.post('/private/retention-plan', form)
    result.value = data.result
  } catch (error) {
    console.error('流失预警方案生成失败:', error)
