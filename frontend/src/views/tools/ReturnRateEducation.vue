<template>
  <ToolDetail :tool-info="toolInfo" :result="result" @submit="handleSubmit">
    <template #inputs>
      <div class="form-row">
        <div class="form-group"><label class="form-label">投入金额（元）</label><input v-model.number="form.investment" type="number" class="form-input" placeholder="活动/推广投入" min="0" /></div>
        <div class="form-group"><label class="form-label">产出金额（元）</label><input v-model.number="form.output" type="number" class="form-input" placeholder="新签学费" min="0" /></div>
      </div>
    </template>
    <template #result>
...
const toolInfo = getToolByCode('return-rate-education')
...
function handleSubmit() {
  ...
  let status = 'warning', statusText = '持平', verdict = '', reference = '教培获客ROI>300%值得持续，<150%需换渠道'

  if (roi >= 500) { status = 'success'; statusText = '高回报'; verdict = '回报非常好！建议加大该渠道投入。' }
  else if (roi >= 300) { status = 'success'; statusText = '值得投入'; verdict = '达到健康线，值得持续投入。' }
