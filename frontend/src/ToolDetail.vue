...
onMounted(async () => {
  quotaStore.setTool(resolvedToolInfo.value.code)
  emit('load-quota')
  await quotaStore.fetchGlobalQuota()
})

const canSubmit = computed(() => {
  if (loading.value || quotaStore.loading) return false
  if (quotaStore.isUnlimited) return true

  const quota = displayQuota.value
  if (!quota) return true

  const remain = Number(quota.remain)
  if (Number.isNaN(remain)) return true
  return remain > 0
})
