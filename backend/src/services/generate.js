...
请给出完整的竞品分析和差异化建议：`,
  ...
  'growth-diagnosis': {
    ...
    templateBuilder: async (formData) => {
      ...
      if (midTermActions.length === 0) {
        midTermActions.push('把短期验证有效的动作沉淀为固定流程，并明确负责人、节奏和复盘指标')
      }
      ...
      problemSection.push('该诊断基于当前问卷与评分生成，若输入样本不足，应先补齐真实经营数据再做重大决策。')
      problemSection.push('增长动作需要结合现金流、团队承接能力和行业周期推进，避免同时启动过多项目导致资源分散。')
  ...
  xiaohongshu: {
    ...
    templateBuilder: async (formData, ind) => {
      ...
      const product = formData.product || `${industry}服务`
      const style = formData.style || '种草'
      const target = formData.target || formData.audience || '目标客户'
      const highlights = formData.highlights || '体验感、专业度和真实效果'
      ...
      return {
        ...
        sections: [
          { title: '标题模板', items: [
            `${industry}${product}真实体验：这3个细节值得收藏`,
            `第一次做${product}前，先看完这份避坑清单`,
            `掏心窝子分享：${industry}${product}怎么选更稳`,
            `${target}做${product}，最容易忽略的是这几点`,
            `${industry}小白必看：从体验到成交的完整攻略`
          ]},
          { title: '正文结构', items: [
          ...
          { title: '正文示例', items: [
            `作为长期关注${industry}消费体验的人，今天用${product}这个例子，分享几个真实避坑点。\n\n` +
            `1. 先看适不适合自己\n ${target}不要只看价格，先确认服务流程、适用场景和预期效果是否匹配。\n\n` +
            `2. 再看交付细节\n 重点关注${highlights}，这些细节比单纯优惠更能影响最终体验。\n\n` +
            `3. 最后看后续承接\n 做完之后有没有跟进、复购提醒或售后说明，决定这次体验是不是值得继续。\n\n` +
            `最后想说：${industry}不是越便宜越好，选对适合自己的方案更重要。\n\n` +
            `觉得有用就收藏起来，有问题可以在评论区交流。\n\n` +
        ...
        actions: [
          { priority: 'critical', title: '补充真实案例', description: '把正文示例中的服务过程、效果反馈和客户疑问替换为门店真实案例', owner: '运营', timeline: '今天' },
          { priority: 'high', title: '准备配图', description: '拍摄或制作3-5张高清配图', owner: '运营', timeline: '2天内' }
        ],
        riskNotes: [
          '小红书内容应避免夸大效果、虚构体验或承诺绝对结果，尤其是美业、教育和健康相关服务。',
          '标题可以提高点击率，但正文必须兑现标题承诺，否则会影响完读、收藏和后续咨询转化。'
        ],
  ...
  'xhs-topic': {
    ...
    templateBuilder: async (formData, ind) => {
      ...
      return {
        ...
        riskNotes: [
          '小红书选题不能只追热点，应确保标题、正文和服务能力一致，避免标题党导致完读和转化下降。',
          '涉及美业、教育、健康等行业时，选题需避免夸大效果、制造焦虑或使用无法证明的绝对化承诺。'
        ],
  ...
  'xhs-conversion': {
    ...
    engineType: 'template',
    templateBuilder: async (formData, ind = {}) => {
      const industry = ind.name || formData.industry || '门店'
      ...
      const goalLabel = goalMap[goal] || goal || '合规引导咨询'

      return {
        summary: `「${industry}」${goalLabel}方案已生成`,
        sections: [
          { title: '安全引流原则', items: [
            '绝不直接在笔记/评论中放微信号/手机号',
            '使用平台内功能承接：私信、群聊、主页',
        ...
        riskNotes: [
          '小红书引流不能直接发布微信号、手机号或诱导绕开平台交易，应优先使用私信、主页、群聊和 POI 等合规承接方式。',
          '引流内容应控制频次并提供真实价值，过度营销会降低账号权重并增加限流、投诉或违规风险。'
        ],
  ...
  schedule: {
    ...
    templateBuilder: async (formData, ind) => {
      ...
      return {
        summaryText: `排班表已生成（${employees.length}人，营业时间${shifts[0].start}-${shifts[2].end}）`,
        days: DAYS,
        schedule,
        hoursStats,
        conflicts,
        tips,
        sections: [
          { title: '排班概览', items: [`员工人数：${employees.length}人`, `周班次数：${totalShifts}次`, `预估周人工成本：¥${Math.round(weeklyCost)}`, `冲突提醒：${conflicts.length}项`] },
          { title: '排班风险', items: conflicts.length ? conflicts : ['当前未发现明显休息日冲突，仍需结合实际客流复核高峰人手。'] },
          { title: '执行建议', items: tips }
        ],
        actions: [
          { priority: 'critical', title: '复核高峰时段人手', description: '按历史客流、订单量和关键岗位要求检查早中晚班是否足够覆盖', owner: '店长', timeline: '发布排班前' },
          { priority: 'high', title: '提前确认员工可出勤', description: '将排班表提前同步给员工，收集请假、换班和特殊约束后再定稿', owner: '排班负责人', timeline: '每周固定时间' }
        ],
        riskNotes: [
          '当前排班为规则生成结果，未自动纳入实时客流、员工技能等级、法定工时和当地劳动法规要求。',
          '预估薪资按统一小时成本简化计算，实际成本需结合底薪、提成、加班、社保和补贴复核。'
        ],
        summary: {
