...
export const CALCULATORS = {
  ...
  roi: {
    ...
    calc: ({ totalInvestment, totalRevenue }) => {
      ...
      return { sections: [
        ...
        { priority: 'critical', title: '建立 ROI 监控日报', description: '每日追踪各渠道 ROI 数据，及时发现异常波动', owner: '运营', timeline: '每日' },
        { priority: 'high', title: '按 ROI 排序渠道并调整预算分配', description: '每周根据 ROI 表现重新分配投放预算，优先高 ROI 渠道', owner: '投放负责人', timeline: '每周' }
      ], riskNotes: [
        'ROI 计算假设所有收入都来自当前投放，实际可能存在自然流量干扰',
        '建议至少追踪 7 天数据再做决策，避免单日波动误判'
  ...
  payback: {
    ...
    calc: ({ totalInvestment, monthlyProfit }) => {
      ...
      return { sections: [
        ...
        { priority: 'critical', title: '设置回本预警线', description: '超过 12 个月自动暂停项目，避免持续亏损', owner: '财务', timeline: '立即' },
        { priority: 'high', title: '分阶段投入', description: '每阶段验证回本周期，达标后再投入下一阶段', owner: '项目负责人', timeline: '项目启动前' }
      ], riskNotes: [
        '回本周期计算基于稳定月利润假设，实际业务可能有季节性波动',
        '建议保留 3 个月运营资金作为安全垫，避免现金流断裂'
  ...
  'break-even-restaurant': {
    ...
    calc: ({ rent, salary, depreciation, otherFixed, dineInPct, dineInVarCost, deliveryPct, deliveryArrivalRate, deliveryVarCost, area, seats, hours, avgTicket, actualRevenue, targetProfit }) => {
      ...
      return {
        ...
        actions: [
          { priority: 'critical', title: '计算当前盈亏平衡点', description: '制定保本经营策略，确保门店不亏损', owner: '店长', timeline: '本周内' },
          { priority: 'high', title: '优化成本结构', description: '降低固定成本或提高客单价，改善盈亏平衡状况', owner: '财务', timeline: '持续' }
        ],
        riskNotes: [
          '盈亏平衡计算基于稳定经营假设，未考虑季节性波动和市场变化',
          '餐饮盈亏受翻台率、客单价、成本控制等多因素影响，需动态调整'
        ],
  ...
  'investment-budget': {
    ...
    calc: ({ storeType, cityLevel, area, isFranchise, franchiseFee, deposit, renovationPerSqm, equipmentCost, initialMaterial, rentMonthly, rentDepositMonths, licenseCost, marketingBudget, reserveMonths, otherCost }) => {
      ...
      return {
        ...
        actions: [
          { priority: 'critical', title: '制定详细的开店投资预算表', description: '明确各项成本和资金需求，确保投资决策合理', owner: '店长', timeline: '本周内' },
          { priority: 'high', title: '建立投资回报监控机制', description: '定期评估实际投入与预期回报，及时调整经营策略', owner: '财务', timeline: '持续' }
        ],
        riskNotes: [
          '投资预算基于行业平均数据，未考虑具体选址和市场环境差异',
          '开店投资受城市级别、商圈位置、品牌类型等多因素影响，需结合实际情况调整'
        ],
  ...
  'food-yield-rate': {
    ...
    calc: ({ rawWeight, netWeight, purchasePrice, wasteSellable, wastePrice }) => {
      ...
      return {
        ...
        actions: [
          { priority: 'critical', title: '建立食材出成率监控表', description: '每日记录实际出成率与标准对比，及时发现异常损耗', owner: '厨师长', timeline: '每日' },
          { priority: 'high', title: '分析低出成率菜品', description: '优化加工流程或调整食材规格，降低整体损耗成本', owner: '采购', timeline: '每周' }
        ],
        riskNotes: [
          '食材出成率受季节、产地、储存条件等因素影响，需动态调整标准',
          '过度追求高出成率可能影响菜品品质和客户体验，需平衡成本与质量'
        ],
  ...
  'cup-efficiency': {
    ...
    calc: ({ dailyCups, operatingHours, staffCount, peakHours, peakCups }) => {
      ...
      return { sections: [
        ...
        { priority: 'critical', title: '分析高峰期出杯瓶颈', description: '优化操作流程和人员配置，确保高峰期服务效率', owner: '店长', timeline: '本周内' },
        { priority: 'high', title: '建立标准化制作流程', description: '减少动作浪费，提高整体出杯效率和产品一致性', owner: '店员', timeline: '持续' }
      ], riskNotes: [
        '出杯效率计算未区分不同产品复杂度，简单饮品和复杂饮品效率差异大',
        '过度追求效率可能影响产品品质和客户体验，需平衡效率与质量'
  ...
  'drink-cost': {
    ...
    calc: ({ drinkName, ingredients }) => {
      ...
      const items = ingredients.map(ing => {
        const cost = ing.cost != null
          ? Number(ing.cost)
          : safeDiv(ing.amount, ing.packageWeight) * ing.packagePrice
        totalCost += cost
      ...
      return { sections: [
        ...
        { priority: 'critical', title: '建立饮品配方成本卡', description: '标准化每款饮品的原料用量和成本，确保成本控制准确', owner: '店长', timeline: '本周内' },
        { priority: 'high', title: '定期评估原料供应商价格', description: '优化配方降低成本，保持合理毛利率', owner: '采购', timeline: '每月' }
      ], riskNotes: [
        '配方成本计算基于当前原料价格，未考虑价格波动和季节性变化',
        '过度追求低成本可能影响饮品品质和客户体验，需平衡成本与品质'
  ...
  'dish-pricing': {
    ...
    calc: ({ storeType, dishes }) => {
      ...
      return {
        ...
        actions: [
          { priority: 'critical', title: '分析现有菜品结构', description: '识别引流菜、利润菜、形象菜的合理配比，优化产品组合', owner: '厨师长', timeline: '本周内' },
          { priority: 'high', title: '基于成本和竞争情况调整菜品定价', description: '优化整体毛利率，平衡客户接受度和利润目标', owner: '店长', timeline: '持续' }
        ],
        riskNotes: [
          '菜品定价受市场竞争、客户心理预期、品牌定位等多因素影响，单一成本定价可能不适用',
          '过度依赖高毛利菜品可能影响客户体验和整体销量，需平衡菜品结构'
        ],
  ...
  'payback-restaurant': {
    ...
    calc: ({ totalInvestment, monthlyProfit }) => {
      ...
      return {
        benchmarks: [
          { metric: '餐饮回本周期', value: `${months.toFixed(1)} 个月`, benchmark: '快餐 8-12，正餐 12-18，咖啡 12-24，茶饮 6-10', status: months <= 18 ? 'ok' : 'below' }
        ],
        sections: [
          { title: '统计口径', items: ['回本周期 = 总投资 / 月净利润。', '适合评估门店或项目多久收回投资，但前提是月净利润具有稳定性。'] },
          { title: '回本周期', items: [`总投资：${formatCurrency(totalInvestment)}`, `月净利润：${formatCurrency(monthlyProfit)}`, `回本周期：${months.toFixed(1)} 个月（约 ${years.toFixed(1)} 年）`, `年化回报：${annualReturn.toFixed(1)}%`] },
          { title: '经营解释', items: [`当前判断：${statusText}`, '餐饮回本不能只看单月账面利润，还要看淡旺季、翻台稳定性、外卖占比和复购黏性。', '如果回本看起来很快，但依赖开业期补贴、短期流量或压缩人工，后续往往会回落。'] },
          { title: '建议', items: months <= 12
            ? ['回本速度较好，可继续验证模型可复制性，再考虑扩店或加码投入。']
            : months <= 18
            ? ['回本周期尚可，重点看利润是否能跨季节保持稳定。']
            : ['优先复盘客单价、翻台率、食材成本率和人工结构，确认门店模型是否过重。', '若长期回本偏慢，应审慎追加装修、设备和租约投入。'] },
          { title: '行业参考', items: ['快餐：8-12个月，正餐：12-18个月，咖啡店：12-24个月', '奶茶/茶饮：6-10个月，小吃/档口：4-8个月'] }
        ],
        actions: [
          { priority: 'critical', title: '复核月净利润稳定性', description: '用最近 3-6 个月净利润重新计算回本周期，剔除开业补贴和一次性收入', owner: '财务', timeline: '本周内' },
          { priority: 'high', title: '拆解缩短回本周期的杠杆', description: '分别测算客单价、翻台率、食材成本率、人工占比每项改善对回本周期的影响', owner: '店长', timeline: '每月' }
        ],
        riskNotes: [
          '回本周期假设月净利润持续稳定，淡旺季、商圈变化、平台补贴取消都会拉长真实回本时间。',
          '若总投资未包含转让费、押金、装修追加、设备折旧和启动期亏损，会低估回本周期。'
        ],
  ...
  'cashflow-restaurant': {
    ...
    calc: ({ currentCash, monthlyRevenue, rent, baseSalary, utilities, otherFixed, foodCostRate, marketingRate, months, upcomingExpenses, memberPrepay }) => {
      ...
      return {
        sections: [
          {
            ...
            items: [
              ...
              `变动成本：¥${totalVariable.toLocaleString()}/月（食材${foodCostRate || 0}%+营销${marketingRate || 0}%）`,
        ...
        actions: [
          { priority: 'critical', title: '建立现金流预警机制', description: '设置关键节点资金储备，避免资金断裂风险', owner: '财务', timeline: '本周内' },
          { priority: 'high', title: '优化应收账款管理', description: '缩短回款周期，提高现金流周转效率', owner: '店长', timeline: '持续' }
        ],
        riskNotes: [
          '现金流计算基于历史收支数据，未考虑季节性波动和突发事件影响',
          '餐饮现金流受节假日、天气、竞争等外部因素影响较大，需结合具体业务场景解读'
        ],
  ...
  'profit-rate-restaurant': {
    ...
    calc: ({ revenue, foodCost, laborCost, rent, otherCost }) => {
      ...
      return {
        benchmarks: [
          { metric: '餐饮净利率', value: `${profitRate.toFixed(1)}%`, benchmark: '优质门店 18%-25%，10%-18% 可控，<10% 需重点优化', status: profitRate >= 10 ? 'ok' : 'below' }
        ],
        sections: [
          ...
          { title: '经营解释', items: [`当前判断：${statusText}`, '餐饮净利率偏低时，往往不是单项成本问题，而是食材、人工、房租和翻台共同挤压后的结果。', '若净利短期好看，也要确认不是靠活动冲量、压缩出品或延后支出换来的表面利润。'] },
          { title: '建议', items: profitRate >= 18
            ? ['净利基础较好，下一步重点看可持续性，避免牺牲出品和服务换利润。']
            : profitRate >= 10
            ? ['优先找出最大成本项，并结合客单价、翻台率和外卖结构一起优化。']
            : ['优先拆分食材、人工、房租和营销/杂费的压力来源。', '若长期低于 10%，需重新审视门店模型和选址是否成立。'] }
        ],
        actions: [
  ...
  'return-rate-restaurant': {
    ...
    calc: ({ investment, return: ret }) => {
      ...
      return {
        benchmarks: [
          { metric: '餐饮 ROI', value: `${roi.toFixed(1)}%`, benchmark: '>200% 值得持续，100%-200% 需优化，<100% 通常偏弱', status: roi >= 100 ? 'ok' : 'below' }
        ],
        sections: [
  ...
  'renewal-rate-education': {
    ...
    calc: ({ expiredStudents, renewedStudents, renewalWindowDays, avgTuition, newStudents, totalStudents, periodLabel }) => {
      ...
      if (renewalGap > 0) {
        suggestions.push(`距离 80% 健康线还差 ${renewalGap} 人，可先集中攻克本周内到期且已完成阶段成果的学员。`)
      }

      const benchmark = renewalWindow <= 30
        ? '短周期班参考：70%-80% 为健康，80% 以上说明续费链路成熟。'
        : '长周期班参考：75%-85% 为健康，85% 以上通常代表教学成果和服务协同稳定。'
  ...
  'class-consumption-rate-education': {
    ...
    calc: ({ totalPurchased, consumed, period, avgTuition, totalStudents, scheduledHours, attendanceRate }) => {
      ...
      const suggestions = []
      if (rate < 50) {
        suggestions.push('消耗率低于 50%，预收款消化严重滞后。建议：1）立即排查排课覆盖率；2）对长期未排课学员主动联系排课；3）推出"密集消课"活动（如暑期/寒假集训）。')
      } else if (rate < 70) {
        suggestions.push('消耗率有提升空间，建议：1）增加排课频次；2）设置出勤提醒和缺课补课机制；3）对连续 2 次缺勤学员进行一对一跟进。')
      } else {
        suggestions.push('消耗速度健康，收入确认节奏良好。继续保持排课密度和学员出勤管理。')
      }
      ...
      const benchmarks = [
        { metric: '课时消耗率', value: `${rate.toFixed(1)}%`, benchmark: '>= 70% 健康，50-70% 正常，< 50% 偏慢', status: rate >= 70 ? 'ok' : rate >= 50 ? 'caution' : 'below' },
        { metric: '标准化月消耗率', value: `${burnRate.toFixed(1)}%`, benchmark: '月度消耗 >= 30% 为良性', status: burnRate >= 30 ? 'ok' : 'below' },
        ...(attendanceGap > 15 ? [{ metric: '出勤缺口', value: `${attendanceGap.toFixed(0)}%`, benchmark: '应控制在 15% 以内', status: 'below' }] : []),
        ...(schedulingGap > 30 ? [{ metric: '排课覆盖率', value: `${(100 - schedulingGap).toFixed(0)}%`, benchmark: '应 >= 70%', status: 'below' }] : [])
      ]
  ...
  'salary-cost-ratio-education': {
    ...
    calc: ({ totalSalary, monthlyRevenue }) => {
      ...
      let statusText = ratio <= 35 ? '人工结构健康' : ratio <= 45 ? '人工结构偏紧' : '人工结构承压'
      return {
        benchmarks: [
          { metric: '教培人工占比', value: `${ratio.toFixed(1)}%`, benchmark: '理想 <=35%，35%-45% 需关注，>45% 通常承压', status: ratio <= 45 ? 'ok' : 'below' }
        ],
        sections: [
          { title: '统计口径', items: ['员工成本占比 = 月薪资总额 / 月营收。', '用于判断人工结构是否健康，不等同于净利润率。'] },
        ...
        riskNotes: [
          '员工成本占比不能只看薪资总额，应结合满班率、续费率和教师负荷综合判断。',
  ...
  'labor-efficiency-education': {
    ...
    calc: ({ monthlyRevenue, teacherCount }) => {
      ...
      let statusText = revenuePerTeacher >= 30000 ? '人效优秀' : revenuePerTeacher >= 20000 ? '人效可控' : '人效偏低'
      return {
        benchmarks: [
          { metric: '教培月人效', value: formatCurrency(revenuePerTeacher.toFixed(0)), benchmark: '>=30000 较优，20000-30000 可控，<20000 需排查', status: revenuePerTeacher >= 20000 ? 'ok' : 'below' }
        ],
        sections: [
        ...
        riskNotes: [
          '人效高不一定代表健康，若依赖少数教师超负荷，后续会影响服务质量和续费。',
  ...
  'venue-utilization-education': {
    ...
    calc: ({ totalHours, bookedHours, rooms }) => {
      ...
      return {
        benchmarks: [
          { metric: '教培场地利用率', value: `${utilization.toFixed(1)}%`, benchmark: '>=70% 较优，50%-70% 一般，<50% 需重点处理', status: utilization >= 50 ? 'ok' : 'below' }
        ],
        sections: [
  ...
  'cac-education': {
    ...
    calc: ({ totalMarketingCost, newStudents, avgTuition }) => {
      ...
      let statusText = cac <= 800 ? '获客优秀' : cac <= 2000 ? '获客可控' : '获客偏贵'
      ...
      return {
        benchmarks: [
          { metric: '教培 CAC', value: formatCurrency(cac.toFixed(0)), benchmark: '线下地推 200-500 元/人，线上投放 500-1500 元/人，转介绍 50-200 元/人', status: cac <= 2000 ? 'ok' : 'below' }
        ],
        sections: [
  ...
  'profit-rate-education': {
    ...
    calc: ({ revenue, teacherCost, venueCost, marketingCost, otherCost }) => {
      ...
      return {
        benchmarks: [
          { metric: '教培净利率', value: `${profitRate.toFixed(1)}%`, benchmark: '15%-25% 常见健康，>25% 较强，<15% 需优化', status: profitRate >= 15 ? 'ok' : 'below' }
        ],
        sections: [
          ...
          { title: '建议', items: profitRate >= 25
            ...
            : ['优先拆分毛利、人工、场地和营销四个核心环节，找出主要压力源。', '若净利长期低于 15%，需重新审视校区模型是否成立。'] }
        ],
        actions: [
  ...
  'return-rate-education': {
    ...
    calc: ({ investment, return: ret }) => {
      ...
      return {
        benchmarks: [
          { metric: '教培获客 ROI', value: `${roi.toFixed(1)}%`, benchmark: '> 300% 值得持续，150%-300% 需优化，<150% 通常要换打法', status: roi >= 150 ? 'ok' : 'below' }
        ],
        sections: [
  ...
  'card-consumption-rate-beauty': {
    ...
    calc: ({ totalCards, consumedCards, period, avgCardValue, totalCustomers, scheduledAppointments, showUpRate }) => {
      ...
      const suggestions = []
      if (rate < 40) {
        suggestions.push('耗卡率低于 40%，预收款消化严重滞后。建议：1）立即排查预约覆盖率；2）对长期未预约客户主动联系；3）推出"限时消卡"活动（如周年庆/节日专项）。')
      } else if (rate < 60) {
        suggestions.push('耗卡率有提升空间，建议：1）增加预约提醒频次；2）设置爽约约束机制；3）对连续 2 次未到店客户进行一对一跟进。')
      } else {
        suggestions.push('耗卡速度健康，收入确认节奏良好。继续保持预约管理和客户到店率。')
      }
      if (estimatedDaysToFinish) {
        const months = Math.round(estimatedDaysToFinish / 30)
      ...
      const benchmarks = [
        { metric: '耗卡率', value: `${rate.toFixed(1)}%`, benchmark: '>= 60% 健康，40-60% 正常，< 40% 偏慢', status: rate >= 60 ? 'ok' : rate >= 40 ? 'caution' : 'below' },
  ...
  'salary-cost-ratio-beauty': {
    ...
    calc: ({ totalSalary, monthlyRevenue }) => {
      ...
      let statusText = ratio <= 30 ? '人工结构健康' : ratio <= 40 ? '人工结构偏紧' : '人工结构承压'
      return {
        benchmarks: [
          { metric: '美业人工占比', value: `${ratio.toFixed(1)}%`, benchmark: '<=30% 较优，30%-40% 需关注，>40% 承压', status: ratio <= 40 ? 'ok' : 'below' }
        ],
        sections: [
  ...
  'labor-efficiency-beauty': {
    ...
    calc: ({ monthlyRevenue, employeeCount }) => {
      ...
      let statusText = revenuePerEmployee >= 30000 ? '人效优秀' : revenuePerEmployee >= 20000 ? '人效可控' : '人效偏低'
      return {
        benchmarks: [
          { metric: '美业月人效', value: formatCurrency(revenuePerEmployee.toFixed(0)), benchmark: '>=30000 较优，20000-30000 可控，<20000 需排查', status: revenuePerEmployee >= 20000 ? 'ok' : 'below' }
        ],
        sections: [
  ...
  'conversion-rate-beauty': {
    ...
    calc: ({ visitors, converted, trialVisitors, trialConverted, newVisitors, newConverted, avgOrderValue, totalRevenue }) => {
      ...
      let statusText = overallRate >= 40 ? '优秀' : overallRate >= 25 ? '一般' : '偏低'
      ...
      if (overallRate < 25) {
        diagnoses.push(`整体转化率低于 25%，意味着每 4 个进店客户只有不到 1 个成交，大量客流在流失。`)
      }
      ...
      if (overallRate < 25) {
        ...
        suggestions.push('转化率优秀，说明门店的接待能力和项目吸引力较强。可以适度加大引流投入扩大规模。')
      }
      ...
      const benchmarks = [
        { metric: '整体转化率', value: `${overallRate.toFixed(1)}%`, benchmark: '美业 >= 30% 健康，>= 40% 优秀', status: overallRate >= 30 ? 'ok' : 'below' },
        ...(trialRate !== null ? [{ metric: '体验转化率', value: `${trialRate.toFixed(1)}%`, benchmark: '>= 40% 为良好', status: trialRate >= 40 ? 'ok' : trialRate >= 25 ? 'caution' : 'below' }] : []),
        ...(newRate !== null ? [{ metric: '新客转化率', value: `${newRate.toFixed(1)}%`, benchmark: '>= 25% 为健康', status: newRate >= 25 ? 'ok' : 'below' }] : []),
        ...(oldRate !== null ? [{ metric: '老客复购转化率', value: `${oldRate.toFixed(1)}%`, benchmark: '>= 50% 为健康', status: oldRate >= 50 ? 'ok' : oldRate >= 35 ? 'caution' : 'below' }] : [])
      ]
  ...
  'profit-rate-beauty': {
    ...
    calc: ({ revenue, productCost, laborCost, rent, otherCost }) => {
      ...
      return {
        benchmarks: [
          { metric: '美业净利率', value: `${profitRate.toFixed(1)}%`, benchmark: '10%-20% 常见可控，>20% 较强，<10% 需重点优化', status: profitRate >= 10 ? 'ok' : 'below' }
        ],
        sections: [
          ...
          { title: '建议', items: profitRate >= 20
            ...
            : ['优先拆分项目结构、人工、房租和耗材，确认真正的利润压力源。', '若长期低于 10%，需重新审视门店模型是否成立。'] }
        ],
        actions: [
  ...
  'return-rate-beauty': {
    ...
    calc: ({ investment, return: ret }) => {
      ...
      return {
        benchmarks: [
          { metric: '美业 ROI', value: `${roi.toFixed(1)}%`, benchmark: '>200% 值得持续，150%-200% 需优化，<150% 通常偏弱', status: roi >= 150 ? 'ok' : 'below' }
        ],
        sections: [
  ...
  'repurchase-rate-beauty': {
    ...
    calc: ({ totalCustomers, repurchasedCustomers, periodDays, serviceCycleDays, avgOrderValue, dormantCustomers }) => {
      ...
      return {
        ...
        benchmarks: [
          { metric: `${observedDays}天复购率`, value: `${rate.toFixed(1)}%`, benchmark: '生活美容 25%-35%，疗程型项目 35%-50%，高频轻项目可更高', status: rate >= 30 ? 'ok' : 'below' }
        ],
        sections: [
  ...
  'ltv-beauty': {
    ...
    calc: ({ avgOrderValue, purchaseFrequency, customerLifespan, cac, serviceGrossMargin, retentionCost }) => {
      ...
      let status, statusText
      if (ltvCacRatio !== null) {
        if (ltvCacRatio >= 5) { status = 'success'; statusText = 'LTV/CAC 优秀' }
        else if (ltvCacRatio >= 3) { status = 'success'; statusText = 'LTV/CAC 健康' }
        ...
        statusText = grossMarginLtv > 5000 ? '客户价值优秀' : grossMarginLtv > 2000 ? '客户价值一般' : '客户价值偏低'
      }

      const diagnoses = []
      if (cacInput > 0 && ltvCacRatio !== null && ltvCacRatio < 3) {
        diagnoses.push(`当前 LTV/CAC = ${ltvCacRatio.toFixed(1)}，低于行业健康线 3。获客投入过高或客户价值不足，需要二选其一优化。`)
      }
      if (margin < 50) {
      ...
      if (ltvCacRatio !== null && ltvCacRatio < 3) {
        ...
        suggestions.push('获客投入健康，建议：1）适度加大投放扩大规模；2）建立会员体系进一步延长客户生命周期；3）提升高毛利项目的推荐率。')
      }
      ...
      const benchmarks = [
        { metric: '美业 LTV/CAC', value: ltvCacRatio !== null ? ltvCacRatio.toFixed(1) : '未填 CAC', benchmark: '>= 3 健康，>= 5 优秀', status: ltvCacRatio !== null ? (ltvCacRatio >= 3 ? 'ok' : 'below') : 'info' },
        { metric: '毛利口径 LTV', value: formatCurrency(grossMarginLtv), benchmark: '美业客户 LTV（毛利）通常 3000-15000 元', status: grossMarginLtv >= 3000 ? 'ok' : 'below' },
      ...
      return {
        benchmarks,
        sections: [
          { title: '统计口径', items: ['LTV（毛利口径）= 客单价 x 年消费频次 x 生命周期 x 服务毛利率。', 'LTV/CAC = 毛利口径 LTV / 获客成本，美业行业基准 >= 3 为健康。'] },
  ...
  'repurchase-rate': {
    ...
    calc: ({ totalCustomers, repeatCustomers, period, avgRepeatInterval, avgOrderValue, newCustomerCost }) => {
      ...
      return {
        benchmarks: [
          { metric: '餐饮复购率', value: `${rate.toFixed(1)}%`, benchmark: '快餐 25%-40%，正餐 20%-35%，火锅 30%-45%，奶茶 35%-55%', status: rate >= 20 ? 'ok' : 'below' }
        ],
        sections: [
  ...
  'ltv-restaurant': {
    ...
    calc: ({ avgOrder, frequency, retentionMonths, grossMargin, cac }) => {
      ...
      return {
        ...
        benchmarks: [
          { metric: '餐饮 LTV/CAC', value: ltvCacRatio.toFixed(1), benchmark: '>= 3 健康，>= 5 优秀', status: ltvCacRatio >= 3 ? 'ok' : 'below' }
        ],
        sections: [
  ...
  'ltv-education': {
    ...
    calc: ({ hourlyFee, monthlyHours, retentionMonths, extraIncomePct, cac }) => {
      ...
      return {
        ...
        benchmarks: [
          { metric: '教培 LTV/CAC', value: ltvCacRatio.toFixed(1), benchmark: '>= 3 健康，>= 5 优秀', status: ltvCacRatio >= 3 ? 'ok' : 'below' }
        ],
        sections: [
